...
export async function loadWordSpeech(text: string): Promise<WordSpeechClips> {
  ...
  if (missing.length) throw new Error(`No spoken clips for ${missing.slice(0, 5).join(", ")}${missing.length > 5 ? ", ..." : ""}. Turn off Three repeats + spoken answer to listen to these words in Morse.`);
