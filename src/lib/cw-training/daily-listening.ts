...
export const DAILY_LISTENING_TITLE = "Bob's 77 Words";
...
export function createDailyListeningBlock(resource: TrainingResource, now: string, id: string) {
  const task: TrainingTask = {
    ...
    instructions: "Optional: listen for at least 10 minutes each day. Listen for the sound of whole words and abbreviations. Bob's original recording uses approximately 40 WPM characters with extra word spacing. The supplied word list is available for reference.",
    sourceUrl: resource.url, resourceId: resource.id, optional: true, minutes: 10,
  };
  return {
    id, assignmentId: DAILY_LISTENING_ASSIGNMENT, task,
    resource, startedAt: now, targetMinutes: 10,
