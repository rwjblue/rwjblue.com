...
export function createTrainingReportPanel(root: HTMLElement, options: ReportPanelOptions) {
  ...
  function lcwoSources(result: ReportDraft) {
    ...
    return result.lcwoSources.map(({ run, runs, missingKeys, averageErrorPercent }) => {
      const measurements = [run.score !== undefined ? `Score ${run.score}` : "",
        ...
        averageErrorPercent !== undefined
          ? `Average errors: ${averageErrorPercent}% across ${runs.filter(candidate => candidate.accuracyPercent !== undefined).length} run(s) at these speeds`
          : run.accuracyPercent !== undefined ? `Stored accuracy: ${run.accuracyPercent}%` : "",
        ["letters", "figures", "custom"].includes(run.kind) ? "Assumed group length: 3" : ""].filter(Boolean);
  ...
  function render() {
    ...
    root.innerHTML = `<div class="training-today-heading"><div><p class="eyebrow">Before class</p><h2>Session report</h2></div><p>Send around ${escape(target)}</p></div>
      ...
      <div class="training-card"><h3>LCWO results</h3><p>${snapshot().lcwo?.configured ? "Saved results sync when you open Report if the last sync is at least 5 minutes old. Use Sync LCWO to check sooner. Code groups use average errors at the latest speeds and assume group length 3. Callsign and word scores also fill automatically." : "LCWO syncing has not been connected yet. You can still fill in results below."}</p><button type="button" data-report-action="lcwo-sync"${snapshot().lcwo?.configured ? "" : " disabled"}>Sync LCWO</button><p data-lcwo-status role="status" class="training-small" aria-live="polite"></p>${result.lcwoPractice.estimatedMinutes ? `<p class="training-small" data-lcwo-duration>${result.lcwoPractice.estimatedMinutes} estimated exercise minutes from ${result.lcwoPractice.runs.length} code-group runs (1 minute each). ${result.lcwoPractice.alreadyLoggedMinutes} already covered by saved practice; ${result.lcwoPractice.additionalMinutes} additional minutes count toward your daily totals.</p>` : ""}${result.lcwoSources.length ? `<details class="training-panel" data-lcwo-details><summary>Imported runs and missing details (${result.lcwoSources.length})</summary><div class="training-panel-body"><p class="training-small">Group errors use 100 minus saved accuracy, averaged within the selected dates at matching character and effective speeds. You can edit the assumed length of 3. For Words and Callsigns, add your training speed and error count; maximum copied speed may differ.</p><ul data-lcwo-sources>${lcwoSources(result)}</ul></div></details>` : '<p class="training-small">No imported results selected for this practice window. Results entered with saved practice can also supply report answers.</p>'}</div>
