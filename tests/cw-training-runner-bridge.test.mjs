...
const event = (type, sequence, elapsedSeconds = 0, rest = {}) => ({
  ...
  type, sequence, elapsedSeconds, ...rest,
});
...
test("commands carry only configuration and run identity, and never start playback", () => {
  ...
  assert.deepEqual(runnerConfigureCommand(state), {
    channel: RUNNER_CHANNEL, version: 1, type: "configure", runId: state.runId, settings: settings(),
  });
  assert.deepEqual(runnerStopCommand(state), { channel: RUNNER_CHANNEL, version: 1, type: "stop", runId: state.runId });
...
test("events require a versioned exact bounded numeric schema", () => {
  for (const invalid of [
    null, [], "ready", event("unknown", 0), event("ready", 0, 0, { version: 2 }),
...
test("only started uninterrupted engine time reaching the assignment duration completes one run", () => {
  let state = run();
  ...
  assert.match(runnerResultNote(state), /completed; 900 seconds; Single Call; 13 WPM starting speed; assigned duration 900 seconds; band conditions off; 12 QSOs; Verified Pts 10; verified score 80; NR 1; NIL 1/);
});
...
test("errors preserve only reported engine time and cannot be restarted or upgraded to complete", () => {
  ...
  assert.equal(reduceRunnerEvent(state, event("started", 3)), state);
  assert.equal(reduceRunnerEvent(state, event("results", 4, 900, { reason: "completed", summary: summary() })), state);
  assert.match(runnerResultNote(state), /interrupted \(partial\); 123 seconds; Single Call; 13 WPM starting speed; assigned duration 900 seconds; band conditions off; results unavailable/);
...
test("result notes always include mode and actual settings, with WPX activity and explicit conditions", () => {
  ...
  assert.match(runnerResultNote(state), /stopped \(partial\); 210 seconds; WPX Contest; 18 WPM starting speed; assigned duration 900 seconds; Activity 1; band conditions QRM, FLUTTER/);
...
test("the static adapter accepts the typed commands and rejects extra data or invalid settings", () => {
  ...
  for (const invalid of [
    ...
    { ...runnerStopCommand(run()), version: 2 },
...
function fakeSurface() {
  ...
  const doc = {
    ...
    getElementById(id) {
      if (!elements.has(id)) elements.set(id, { ...eventTarget(), disabled: false, textContent: "", classList: { remove() {} } });
  ...
  const view = {
    ...
    _config: { _config: { farnsworth: true, contest_start_offset_min: 999 }, update_dom() {}, update() {}, updateWPM() {} },
...
test("adapter readiness needs trusted parent/source, settings apply before Run, and commands cannot start audio", async () => {
  ...
  assert.equal(surface.doc.getElementById("wpm").disabled, true);
